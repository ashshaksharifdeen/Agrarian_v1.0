...
#sys.path.append('e:/Agri/Agrarian_v1.0')
import random
from typing import List
from scipy.spatial.distance import cdist
from src.indexes.video_foot import index_cal
...
class PointcloudSet():
    ...
    def findCropsDBSCAN(self):
        with o3d.utility.VerbosityContextManager(
            ...
            """            
        labels = np.array(
                self.map.cluster_dbscan(eps=0.08, min_points=10, print_progress=True))
...
if __name__ == """__main__""":
    ...
    point_cloud= np.loadtxt("E:/point_finalize/Nintavur/"+"map1.xyz",skiprows=1,delimiter=',')
    ...
    solutions =  prompt_design(round(analysis.average_height,3),round(analysis.density,3),round(lai_,3),round(leaf_nitrogen_con,3),round(exg_green,3),round(ndi,3))
    ...
    host="agrarian-instance-1.camkk37xnb4t.ap-southeast-2.rds.amazonaws.com",
    user="admin",
    password="Agrarian#007*#",
    database="Agrarian")
    ...
    LNC = ("LNC",round(leaf_nitrogen_con,3))
    ...
    sql7=  'INSERT INTO Map (Solution) VALUES (%s)'
    #for solution in solutions:
    json_ob = json.dumps(solutions)
       
    mycursor.execute(sql7, (json_ob,))
    print(mycursor.rowcount, "Solution record inserted.")
    mydb.commit()
